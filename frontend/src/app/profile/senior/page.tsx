...
export default function SeniorProfile() {
    const [formData, setFormData] = useState({
      ...
      const handleSubmit = async (e) => {
        e.preventDefault()
        try {
          const response = await fetch('/api/users/senior-profile', {
            method: 'POST',
            headers: {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input id="prefecture" name="prefecture" value={formData.prefecture} onChange={handleChange} required />
      </div>
      <div>
      ...
      <div>
        ...
        <textarea id="background" name="background" value={formData.background} onChange={handleChange} required />
      </div>
      <div>
